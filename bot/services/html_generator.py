...
GENERATION_PROMPT = """Ты — опытный дизайнер коммерческих предложений. Твоя задача — создать HTML-документ КП для клиента.

## ИНФОРМАЦИЯ О ПРОДУКТЕ

WorkHere — ATS-система для рекрутинга:
- Единая база кандидатов
- Интеграции: HH.ru, Авито, SuperJob
...
## СТРУКТУРА КП (6 разделов)

1. **Текущая ситуация** — диагноз: что сейчас у клиента, какие боли
2. **Как WorkHere решит задачи** — решения под боли клиента
3. **Стоимость** — таблица + расчёт для команды
4. **Внедрение** — 4 шага (настройка, интеграции, импорт, обучение)
5. **Что получит команда** — 2 колонки: для рекрутеров и руководителей
6. **Следующие шаги** — согласование, договор, доступ, запуск

## ПРАВИЛА ДИЗАЙНА

1. Документ должен занимать 2-3 ПОЛНЫЕ страницы A4 — никаких пустот!
2. Между разделами — маскоты с фразами (3 маскота на документ)
3. Если на последней странице остаётся пустота — добавь больше контента или больший маскот
4. Пункты списков — КОРОТКИЕ, максимум 15-20 слов
5. Раздел 5 — по 2-3 коротких пункта в каждой колонке
6. Персонализация — название компании минимум 5 раз в документе
7. В конце документа добавь блок с контактами или призыв к действию

## CSS СТИЛИ (используй эти классы)

{css_styles}

## МАСКОТЫ (вставляй между разделами)
...
async def generate_html_proposal(
    ...
    logger.info(f"Generating HTML proposal via GPT-4o ({len(transcript)} chars transcript)...")
