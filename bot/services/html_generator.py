...
## ПРАВИЛА ДЛЯ МАСКОТОВ (ВАЖНО!)

1. Маскоты используются для ЗАПОЛНЕНИЯ ПУСТОТЫ внизу страницы
2. Если внизу страницы остаётся пустое пространство (более 15% высоты) — ВСТАВЬ маскота
3. Маскот размещается ПОСЕРЕДИНЕ пустого пространства, НЕ прилеплен к тексту сверху и НЕ прилеплен к низу страницы
4. Можно использовать несколько маскотов — по одному на каждую страницу с пустотой
...
1. Раздел 2 ДОЛЖЕН уместиться на ОДНОЙ странице!
2. Подразделы 2.1, 2.2, 2.3... — но КОМПАКТНО
3. Каждый подраздел — 2-3 коротких пункта, не больше
4. Выделяй ТОЛЬКО самое важное, что обсуждали на встрече
5. Не перечисляй весь функционал — только релевантное для клиента
