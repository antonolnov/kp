...
PAGE_REVIEW_PROMPT = """Проверь эту страницу коммерческого предложения. Будь СТРОГИМ!

КРИТИЧЕСКИЕ ОШИБКИ (is_ok = false):
1. Более 30% страницы пустое — КРИТИЧНО! Нужен большой маскот или больше контента
2. Маскот маленький и не заполняет пустоту — КРИТИЧНО! Маскот должен быть большим
3. Маскот вверху страницы, а внизу пустота — КРИТИЧНО! Маскот должен быть внизу
4. Футер дублируется (есть и вверху и внизу) — КРИТИЧНО!

ОБЫЧНЫЕ ОШИБКИ:
5. Элементы слипаются, нет отступов между блоками
6. Маркировка списков наезжает на текст
7. Раздел обрезан/перенесён на следующую страницу

Ответь JSON:
{
    "is_ok": true/false,
    "empty_space_percent": число от 0 до 100,
    "issues": ["проблема 1", "проблема 2"],
    "suggestions": ["исправление 1", "исправление 2"]
}

Будь СТРОГИМ! Пустая страница с маленьким котом — это ПЛОХО, is_ok = false.
Отвечай ТОЛЬКО JSON."""
